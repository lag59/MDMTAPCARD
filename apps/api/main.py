...
app.include_router(public_gallery.router, tags=["public-gallery"])
...
async def _schema_guard_startup() -> None:
    ...
    async with engine.begin() as conn:
        ...
        # Social content system (Phase 1) — scoped by tenant_id + business_id.
        await conn.execute(
            text(
                """
        ...
        await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_social_connections_business_id ON social_connections(business_id)"))
        ...
        await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_website_feeds_business_id ON website_feeds(business_id)"))
